    ...
    public class NetConvertHelper
    {
        ...
        public static unsafe void WriteArray<T>(byte* ptr, ref int offset, T[] value) where T : struct
        {
            if (value != null)
            {
                ...
                Unsafe.Write(ptr + offset, (SIZE)len);
                offset += LENGTH;
            ...
            {
                Unsafe.Write<SIZE>(ptr + offset, 0);
                offset += LENGTH;
            }
        }
        ...
        {
            var arrayLen = Unsafe.Read<SIZE>(ptr + offset);
            offset += LENGTH;
            if (arrayLen > 0)
            {
        ...
        public static unsafe void Write(byte* ptr, ref int offset, string value)
        {
            if (!string.IsNullOrEmpty(value))
            {
                ...
                Unsafe.Write(ptr + offset, (SIZE)count);
                offset += LENGTH + count;
#else
                ...
                fixed (char* ptr1 = value)
                {
                    ...
                    Unsafe.Write(ptr + offset, (SIZE)byteCount);
                    offset += LENGTH;
        ...
        {
            var value = Unsafe.Read<T>(ptr + offset);
            offset += Unsafe.SizeOf<T>();
            return value;
        }
        ...
        {
            var count = Unsafe.Read<SIZE>(ptr + offset);
            offset += LENGTH;
            if (count > 0)
            {
#if CORE
