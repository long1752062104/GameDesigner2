...
namespace Net.System
{
    ...
    [Serializable]
    public class FastListSafe<T> : IList<T>, ICollection<T>, IEnumerable<T>, IEnumerable, IList, ICollection, IReadOnlyList<T>, IReadOnlyCollection<T>
    {
        public FastListSafe()
        {
            _items = _emptyArray;
        ...
        public FastListSafe<TOutput> ConvertAll<TOutput>(Converter<T, TOutput> converter)
        {
            if (converter == null)
            {
                ThrowHelper.ThrowArgumentNullException(ExceptionArgument.converter);
            }
            FastListSafe<TOutput> list = new FastListSafe<TOutput>(_size);
            for (int i = 0; i < _size; i++)
            {
        ...
        {
            internal Enumerator(FastListSafe<T> list)
            {
                this.list = list;
                index = 0;
            ...
            private readonly FastListSafe<T> list;
